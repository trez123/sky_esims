((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,A,C={a5j:function a5j(d,e,f,g){var _=this
_.a=d
_.b=e
_.c=f
_.d=g},a3O:function a3O(){}},D
B=c[0]
A=c[2]
C=a.updateHolder(c[48],C)
D=c[50]
C.a5j.prototype={
k(d,e){var x=this
if(e==null)return!1
if(x===e)return!0
if(!(e instanceof C.a5j))return!1
return e.a===x.a&&e.b===x.b&&e.c===x.c&&e.d===x.d},
j(d){var x=this
return"scrollOffset: "+B.i(x.a)+" precedingScrollExtent: "+B.i(x.b)+" viewportMainAxisExtent: "+B.i(x.c)+" crossAxisExtent: "+B.i(x.d)},
gn(d){var x=this
return B.Q(x.a,x.b,x.c,x.d,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a,A.a)}}
C.a3O.prototype={
gzI(){return null},
pR(d,e){var x
this.gzI()
x=this.gzH()
x.toString
return x*e},
aaI(d,e){var x,w,v
this.gzI()
x=this.gzH()
x.toString
if(x>0){w=d/x
v=A.d.aR(w)
if(Math.abs(w*x-v*x)<1e-10)return v
return A.d.hk(w)}return 0},
Sn(d,e){var x,w,v
this.gzI()
x=this.gzH()
x.toString
if(x>0){w=d/x-1
v=A.d.aR(w)
if(Math.abs(w*x-v*x)<1e-10)return Math.max(0,v)
return Math.max(0,A.d.h_(w))}return 0},
aC_(d,e){var x,w
this.gzI()
x=this.gzH()
x.toString
w=this.y1.guX()
return w*x},
Co(d){var x
this.gzI()
x=this.gzH()
x.toString
return y.b.a(B.B.prototype.ga2.call(this)).aAF(x,x)},
c0(){var x,w,v,u,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f,e,d,a0,a1=this,a2=null,a3=y.b.a(B.B.prototype.ga2.call(a1)),a4=a1.y1
a4.R8=!1
x=a3.d
w=x+a3.z
v=w+a3.Q
a1.dV=new C.a5j(x,a3.e,a3.y,a3.w)
u=a1.aaI(w,-1)
t=isFinite(v)?a1.Sn(v,-1):a2
if(a1.ak$!=null){s=a1.a3_(u)
a1.rl(s,t!=null?a1.a30(t):0)}else a1.rl(0,0)
if(a1.ak$==null)if(!a1.MW(u,a1.pR(-1,u))){r=u<=0?0:a1.aC_(a3,-1)
a1.dy=D.ls(a2,!1,a2,a2,r,0,0,r,a2)
a4.rv()
return}q=a1.ak$
q.toString
q=q.b
q.toString
p=y.c
q=p.a(q).b
q.toString
o=q-1
n=a2
for(;o>=u;--o){m=a1.a5Z(a1.Co(o))
if(m==null){a1.dy=D.ls(a2,!1,a2,a2,0,0,0,0,a1.pR(-1,o))
return}q=m.b
q.toString
p.a(q).a=a1.pR(-1,o)
if(n==null)n=m}if(n==null){q=a1.ak$
q.toString
l=q.b
l.toString
l=p.a(l).b
l.toString
q.hn(a1.Co(l))
l=a1.ak$.b
l.toString
p.a(l).a=a1.pR(-1,u)
n=a1.ak$}q=n.b
q.toString
q=p.a(q).b
q.toString
o=q+1
q=B.w(a1).i("aH.1")
l=t!=null
for(;;){if(!(!l||o<=t)){k=1/0
break}j=n.b
j.toString
m=q.a(j).aS$
if(m!=null){j=m.b
j.toString
j=p.a(j).b
j.toString
j=j!==o}else j=!0
if(j){m=a1.a5X(a1.Co(o),n)
if(m==null){k=a1.pR(-1,o)
break}}else m.hn(a1.Co(o))
j=m.b
j.toString
p.a(j)
i=j.b
i.toString
j.a=a1.pR(-1,i);++o
n=m}q=a1.dz$
q.toString
q=q.b
q.toString
q=p.a(q).b
q.toString
h=a1.pR(-1,u)
g=a1.pR(-1,q+1)
k=Math.min(k,a4.OW(a3,u,q,h,g))
f=a1.yE(a3,h,g)
e=a1.Ew(a3,h,g)
d=x+a3.r
a0=isFinite(d)?a1.Sn(d,-1):a2
a1.dy=D.ls(e,a0!=null&&q>=a0||x>0,a2,a2,k,f,0,k,a2)
if(k===g)a4.R8=!0
a4.rv()}}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(C.a5j,B.x)
x(C.a3O,D.my)})()
B.cL(b.typeUniverse,JSON.parse('{"a3O":{"my":[],"dq":[],"aH":["H","fy"],"B":[],"aN":[]}}'))
var y={b:B.F("jC"),c:B.F("fy")}};
(a=>{a["A3AAD81vJ8e1jkAKIGEtbIjIJ6Q="]=a.current})($__dart_deferred_initializers__);
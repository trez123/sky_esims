((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,B={
baK(d,e,f,g){return new B.Fb(e,f,g,d,null)},
Fb:function Fb(d,e,f,g,h){var _=this
_.c=d
_.e=e
_.f=f
_.CW=g
_.a=h},
a9l:function a9l(d,e,f,g){var _=this
_.w=_.r=_.f=_.e=_.d=$
_.x=d
_.y=e
_.dz$=f
_.bo$=g
_.c=_.a=null},
aO7:function aO7(d){this.a=d},
Sd:function Sd(){},
Fa:function Fa(d,e){this.a=d
this.b=e},
bgo(d,e){var x,w=e.gQ_(),v=y.g
w=A.ak(["name",w,"url","/"],v,v)
x=e.ga8f()
x=A.ak(["name",x,"url","/profile"],v,v)
E.b7n(A.b([w,x,A.ak(["name",d,"url","/profile/"+A.fF(d.toLowerCase()," ","-")],v,v)],y.o))}},D,F
A=c[0]
C=c[2]
E=c[42]
B=a.updateHolder(c[32],B)
D=c[89]
F=c[60]
B.Fb.prototype={
ac(){var x=y.c
return new B.a9l(A.b([],x),A.b([],x),null,null)}}
B.a9l.prototype={
aw(){this.V3()
this.aZ()},
b6(d){this.V3()
this.bs(d)},
V3(){var x=this,w=x.a
x.x=w.e
x.y=w.f
x.f=x.ana()
w=x.a
w.toString
x.r=new B.Fa(C.dF,F.qh)
x.w=new B.Fa(C.m6,D.Mj)
w=w.c
w.aLS(0,!0)
x.e=w
x.d=A.cN(C.mS,w,null)},
F(d){var x=this.d
x===$&&A.a()
return A.h5(x,new B.aO7(this),null)},
ana(){var x,w,v=this.a
if(v.e.length!==v.f.length)throw A.c(A.dc("primaryColors.length != secondaryColors.length"))
x=A.b([],y.l)
for(w=0;v=this.x,w<v.length;++w)x.push(new A.f5(v[w],this.y[w]))
return x},
ams(d){var x,w=A.b([],y.c),v=0
for(;;){x=this.f
x===$&&A.a()
if(!(v<x.length))break
x=x[v].ag(0,d.gp(0))
x.toString
w.push(x);++v}return w},
m(){var x=this.e
x===$&&A.a()
x.m()
this.ahb()}}
B.Sd.prototype={
bW(){this.cw()
this.cn()
this.eA()},
m(){var x=this,w=x.bo$
if(w!=null)w.K(0,x.gem())
x.bo$=null
x.aX()}}
B.Fa.prototype={
f9(d){var x=A.baJ(this.a,this.b,d)
x.toString
return x}}
var z=a.updateTypes([])
B.aO7.prototype={
$2(d,e){var x,w,v,u=null,t=this.a
t.a.toString
x=t.r
x===$&&A.a()
w=t.d
w===$&&A.a()
w=x.ag(0,w.gp(0))
t.a.toString
x=t.w
x===$&&A.a()
x=x.ag(0,t.d.gp(0))
v=t.ams(t.d)
return A.bm(u,t.a.CW,C.u,u,u,new A.bH(u,u,u,u,u,new A.fd(w,x,C.ax,v,u,u),C.N),u,u,u,u,u,u,u)},
$S:773};(function aliases(){var x=B.Sd.prototype
x.ahb=x.m})();(function inheritance(){var x=a.mixinHard,w=a.inherit
w(B.Fb,A.W)
w(B.Sd,A.a_)
w(B.a9l,B.Sd)
w(B.aO7,A.hu)
w(B.Fa,A.aS)
x(B.Sd,A.dZ)})()
A.cM(b.typeUniverse,JSON.parse('{"Fb":{"W":[],"d":[]},"a9l":{"a_":["Fb"]},"Fa":{"aS":["dD"],"aA":["dD"],"aA.T":"dD","aS.T":"dD"}}'))
var y={c:A.G("v<O>"),l:A.G("v<f5>"),o:A.G("v<aq<h,h>>"),g:A.G("h")};(function constants(){D.Mj=new A.dD(1,1)})()};
(a=>{a["sZnuk+8KhitNsEy8aVpt6f3kg+s="]=a.current})($__dart_deferred_initializers__);